...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            $stmt->execute([$code, $type, $value, $max_uses, $usage_limit, $times_used, $used_count, $expiry_date, $min_purchase, $_GET['id']]);
            $_SESSION['success_message'] = 'Coupon modifié avec succès !';
            header('Location: admin_coupons.php');
            exit();
...
$page_title = 'Modifier un Coupon - Administration';
$admin_active = 'coupons';
$admin_title = 'Modifier un Coupon';
$admin_actions = [];
?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="expiry_date" required
                               value="<?= date('Y-m-d\TH:i', strtotime($coupon['expiry_date'])) ?>"
                               class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-primary focus:outline-none transition-all">
                    </div>
